...
function StatsScreen() {
  ...
  const calculateExpensePercentage = () => {
    ...
    listItems.forEach((item) => {
      if (!item.isPlus) {
        if (categoryTotals[item.category]) {
          categoryTotals[item.category] += parseFloat(item.amount.replace(/,/g, ''));
        } else {
          categoryTotals[item.category] = parseFloat(item.amount.replace(/,/g, ''));
        }
      }
    ...
    const expensePercentages = Object.entries(categoryTotals).map(([category, amount]) => ({
      name: category,
      amount,
        ...
        <FlatList
          ListHeaderComponent={
            <>
              ...
              />
              <ExpensePieChart data={listItems} /> */}
            </>
          }
          data={listItems}
