...
import CustomBarChart from "../../RecordScreenComponents/CustomBarChart";
...
function StatsScreen({ userInfo }) {
  ...
  const calculateExpensePercentage = () => {
    ...
    const totalExpense = sortedCategories.reduce((total, [, amount]) => total + amount, 0);
  ...
  const calculateIncomePercentage = () => {
    ...
    const data_income = sortedCategories.map(([category, amount], index) => {
      ...
      console.log("data_income", data_income);
          ...
          <View style={styles.chartcontainer}>
            ...
            <BarChart
              ...
              showValuesOnTopOfBars={({ values }) =>
                values.map((value) => value !== 0 ? value.toLocaleString() : '')
              }
